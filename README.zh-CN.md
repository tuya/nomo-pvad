...
[<kbd>English</kbd>](README.md) | **简体中文**
...
- 本包含**推理代码 + 示例音频**（不含训练代码），版本 `1.1`（对应 tag `release-1.1`）。
...
```
示意输出（节选；`p` 为目标说话人概率，末尾是逐 160ms 的时间轴）：
```text
▶ 流式检测中（每 160ms 出一帧）…
    0.16s  p=0.92  █ 目标说话人
    0.32s  p=0.90  █ 目标说话人
...
for chunk_f32 in stream_160ms_chunks:           # 每 160ms (2560 采样 @16k)
...
> 随仓库分发的 `nomo_pvad.pt` 已包含微调后的骨干。原始完整 ERes2NetV2（含 pooling）用于提取注册声纹；若注册在云端或配网阶段完成，可通过 `sess.set_enrollment_emb(...)` 传入预先计算的 192 维向量。当前 Python 封装在初始化时也会加载原始 ERes2NetV2 检查点，即使使用预先计算的向量也如此。离线使用需提前缓存该模型，或通过 `eres2netv2_dir`（命令行：`--eres2netv2_dir`）指定其本地目录；否则首次运行会自动下载。
...
维护者操作步骤见 [GitHub 发布操作手册](RELEASING.md)。版本通过 Git 标签发布，工作流不创建 GitHub Release 页面或上传附件。
